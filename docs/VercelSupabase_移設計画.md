...
- 環境変数に `NEXT_PUBLIC_SUPABASE_URL`、`NEXT_PUBLIC_SUPABASE_ANON_KEY`、`SUPABASE_SERVICE_ROLE_KEY`、`APP_PASSWORD` を設定。  
...
3. **環境変数**（Vercel の Project → Settings → Environment Variables）:
   ...
   - `SUPABASE_SERVICE_ROLE_KEY`: Supabase の **service_role** key（リセット機能用。RLS 有効時は必須）
